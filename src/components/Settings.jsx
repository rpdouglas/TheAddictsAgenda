            ...
            <div className="space-y-8">
                            ...
                            {isPinSet ? (
                                    ...
                                    {isRemovingPin ? (
                                        <form onSubmit={(e) => { e.preventDefault(); handleConfirmRemovePin(); }} className="space-y-3 p-3 bg-red-50 rounded-lg">
                                            ...
                                            <div className="flex gap-2">
                                                <button type="button" onClick={() => { setIsRemovingPin(false); setRemovalPinAttempt(''); setPinMessage(''); }} className="flex-grow bg-gray-500 text-white font-bold py-3 px-8 rounded-lg shadow-md hover:bg-gray-600">Cancel</button>
                                                <button type="submit" disabled={removalPinAttempt.length < 4} className="flex-grow bg-red-600 text-white font-bold py-3 px-8 rounded-lg shadow-md hover:bg-red-700 disabled:bg-gray-400">Confirm Removal</button>
                                            </div>
                                        </form>
                                    ) : (
                                        <button onClick={() => setIsRemovingPin(true)} className="w-full mt-2 bg-red-500 text-white font-bold py-3 px-8 rounded-lg shadow-md hover:bg-red-600">Remove PIN Lock</button>
                                    )}
                                </div>
                            ) : (
                                <div className="space-y-3">
                                    ...
                                    <input
                                        ...
                                        maxLength={6}
                                    />
                                    <button onClick={handleSetPin} disabled={pin.length < 4 || pin !== confirmPin} className="w-full bg-teal-600 text-white font-bold py-3 px-8 rounded-lg shadow-md hover:bg-teal-700 disabled:bg-gray-400">Set PIN Lock</button>
                                </div>
                            )}
                        </>
                    )}
                ...
                <div className="p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex items-start mt-4">
                        ...
                        <label htmlFor="journal-change" className="ml-3 text-sm text-gray-600 cursor-pointer">
                            ...
                            <span className="block text-xs text-gray-500">This can help you reflect on why your date is being adjusted.</span>
                        </label>
                    </div>
                    <button onClick={handleSaveDate} className="w-full mt-4 bg-teal-600 text-white font-bold py-3 px-8 rounded-lg shadow-md hover:bg-teal-700">Update Date</button>
                ...
                <div className="p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                    ...
                    <button
                        ...
                        disabled={exporting}
                        className="w-full flex items-center justify-center gap-2 bg-yellow-600 text-white font-bold py-3 px-8 rounded-lg shadow-md hover:bg-yellow-700 disabled:bg-gray-400"
                    >
